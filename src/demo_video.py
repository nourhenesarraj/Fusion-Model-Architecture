...
def val_epoch(model: SSD300, input_size: Tuple,fdz_case: str, vis_vid : str, lwir_vid : str, min_score: float = 0.1) -> Dict:
    ...
    while True:
      ...
      img_vis = vis.to(device)
      img_lwir = lwir.to(device)
      ...
      i += 4
      # Forward prop.
      predicted_locs, predicted_scores = model(img_vis, img_lwir)
...
@timing
def run_inference(model_path: str, fdz_case: str ,vis_vid : str, lwir_vid : str ) -> Dict:
    ...
    results = val_epoch(model,input_size,fdz_case,vis_vid, lwir_vid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--videovis', type=str, default='/content/drive/MyDrive/robot-videos*/vis_vid_1.mp4',
                        help='visible video')                   
    parser.add_argument('--result-dir', type=str, default='/content/drive/MyDrive/video_demo_results',
    ...
    # Run inference
    results = run_inference(arguments.model_path, fdz_case, arguments.videovis, arguments.videolwir)
